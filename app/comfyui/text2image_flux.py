...
def load_workflow_from_json(file_path, custom_prompt=None, negative_prompt=None, image_filename=None):
    ...
    # 处理提示语替换
    if custom_prompt or negative_prompt:
        # 遍历所有节点
        for node_id, node in prompt.items():
            ...
            if node['class_type'] == 'CLIPTextEncode':
                ...
                if 'Positive' in node_title or 'positive' in node_title:
                    ...
                    # 如果没有明确标题，根据原始提示词内容判断
                    original_text = node['inputs'].get('text', '')
                    if original_text.strip() == '' and negative_prompt:
                        # 空提示词通常是负面提示词节点
                        node['inputs']['text'] = negative_prompt
                    elif custom_prompt:
                        # 否则视为正面提示词节点
...
def generate_image_from_url_and_prompt(prompt_text, image_url, workflow_path=None):
    ...
    try:
        ...
        if workflow_path is None:
            workflow_path = os.path.join(os.path.dirname(__file__), 'workflows', 'flux_dev_checkpoint_example.json')
...
def example_usage():
    ...
    prompt = "A smoldering couple clad in ultra-sultry outfits, soaking lazily in a steamy hot spring. The man wears a form-fitting black silk swim trunk that hugs his toned waist, a silver chain glinting softly against his tanned chest, his damp hair falling in messy strands over his forehead. The woman dons a slinky, strapless metallic bronze one-piece swimsuit with cutout details at the sides, accentuating her curves, a delicate gold body chain trailing down her spine, her glossy chestnut waves cascading over her shoulders, dampened by the spring’s mist. They’re nestled close together, limbs tangled gently, the man’s arm slung loosely around the woman’s waist as she leans her head against his shoulder, their faces softened by the warm, hazy steam. The hot spring is surrounded by smooth, dark river rocks and lush clusters of emerald moss, with wispy tendrils of steam curling upward into the crisp night air. Above them, a canopy of twinkling fairy lights is strung between ancient pine trees, casting a warm, romantic glow over the water. In the distance, the faint hoot of an owl mingles with the soft gurgle of the spring, and the inky sky is dotted with a million sparkling stars, completing the dreamy, intimate scene"
    # 负面提示词
