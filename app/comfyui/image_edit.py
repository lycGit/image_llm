...
# 读取workflows文件夹下的image_qwen_image_edit_2509.json文件
def load_workflow_from_json(file_path, custom_prompt=None, negative_prompt=None, image_filename=None):
    ...
    if custom_prompt:
        ...
        for node_id, node in prompt.items():
            # 找到TextEncodeQwenImageEditPlus节点
            if node['class_type'] == 'TextEncodeQwenImageEditPlus':
                # 检查原始提示词内容
                original_prompt = node['inputs'].get('prompt', '')
                if original_prompt.strip() == '':
                    ...
                    # 这是正面提示词节点，替换为自定义提示词
                    node['inputs']['prompt'] = custom_prompt
...
def generate_image_from_url_and_prompt(prompt_text, image_url, workflow_path=None, negative_prompt=None):
    """
    从给定的图片URL和提示词生成新图片
    ...
    try:
        ...
        if workflow_path is None:
            workflow_path = os.path.join(os.path.dirname(__file__), 'workflows', 'image_qwen_image_edit_2509.json')
        ...
        try:
            ...
            # 3. 使用提示词和上传的图片加载工作流
            prompt = load_workflow_from_json(workflow_path, custom_prompt=prompt_text, negative_prompt=negative_prompt, image_filename=image_filename)
...
def main():
    # 定义默认提示语
    default_prompt = "Replace the cat with a dalmatian"
    
    # 定义默认负面提示语
    default_negative_prompt = "low quality, blurry, distorted, ugly, text, watermark"
    ...
    try:
        ...
        # 使用默认提示语和上传的图片加载工作流
        prompt = load_workflow_from_json(workflow_path, custom_prompt=default_prompt, negative_prompt=default_negative_prompt, image_filename=image_filename)
...
def example_usage():
    ...
    # 调用图片生成函数
    result = generate_image_from_url_and_prompt(prompt, image_url, negative_prompt=negative_prompt)
