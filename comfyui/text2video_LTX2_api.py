...
class ComfyUIVideoGenerator:
    ...
    def prepare_prompt(self, workflow_data, custom_prompt=None, image_filename=None, negative_prompt=None, duration_seconds=5):
        """准备用于提交的提示词和工作流配置
        
        参数:
            duration_seconds (int): 视频时长（秒），默认5秒
        """
        # 创建提示对象
        prompt = {}
        
        # 处理不同格式的工作流数据
        # 检查是否是以节点ID为键的字典格式（如video_ltx2_t2v_distilled.json）
        # 支持纯数字ID和带冒号的ID格式（如 "177:101"）
        if isinstance(workflow_data, dict) and all(key.replace(':', '').isdigit() for key in workflow_data.keys()):
            ...
            for node_id, node_data in workflow_data.items():
                ...
                # 处理PrimitiveStringMultiline节点（LTX2工作流中存储提示词的节点）
                elif node_type == 'PrimitiveStringMultiline':
                    node_title = node_data.get('_meta', {}).get('title', '')
                    if "Positive" in node_title or "positive" in node_title:
                        # 正面提示词节点
                        if custom_prompt:
                            prompt[node_id]["inputs"]["value"] = custom_prompt
                            print(f"替换正面提示词: {node_id} - {node_title}")
                    elif "Negative" in node_title or "negative" in node_title:
                        # 负面提示词节点
                        if negative_prompt:
                            prompt[node_id]["inputs"]["value"] = negative_prompt
                            print(f"替换负面提示词: {node_id} - {node_title}")
                
                # 处理PrimitiveInt节点（LTX2工作流中存储帧数量的节点）
                elif node_type == 'PrimitiveInt':
                    node_title = node_data.get('_meta', {}).get('title', '')
                    if "Frame Count" in node_title or "frame_count" in node_title.lower():
                        # 计算帧数量：时长(秒) * 帧率(默认24fps)
                        frame_count = int(duration_seconds * 24)
                        prompt[node_id]["inputs"]["value"] = frame_count
                        print(f"设置视频时长: {duration_seconds}秒，帧数: {frame_count}")
    ...
    def generate_video(self, workflow_path, image_path, custom_prompt=None, negative_prompt=None, save_to_local=True, save_directory=None, duration_seconds=5):
        """生成视频的主函数
        ...
        try:
            ...
            # 2. 上传图片（LTX2纯文本到视频不需要图片输入）
            if image_path is not None:
                image_filename = self.upload_image(image_path)
            else:
                image_filename = None
                print("LTX2是纯文本到视频模型，跳过图片上传步骤")
...
# 视频生成示例函数
def generate_video_example(duration_seconds=30):
    """视频生成示例
    ...
    # 工作流文件路径
    workflow_path = os.path.join(os.path.dirname(__file__), 'workflows', 'video_ltx2_t2v_distilled.json')
    
    try:
        # 自定义提示词
        prompt = """Here's the script with all non-dialogue text converted to English, while keeping the spoken Chinese lines unchanged.

Title: Mom's Lesson

Scene: A messy apartment living room. Afternoon sunlight streams through the half-drawn curtains. A young man in his early twenties (Xiao Jie) sits on the sofa, wearing a loose hoodie, holding his phone, having a video call with his mom.

Shot: Medium shot, the camera facing Xiao Jie directly. Behind him, the wall is plastered with movie posters and sticky notes.

Xiao Jie nods at his phone, his expression shifting from serious to helpless. His mom's voice comes through the speaker, clear but tinged with static.

Mom (voice-over): "……总之啊，小杰，妈妈跟你说过多少次了，交朋友要看清人品，不要跟那些不三不四的人混在一起……"

Xiao Jie sighs, props his phone against a water cup on the coffee table, stands up, and walks toward the kitchen. The camera follows him — but stays in medium shot, without moving.

He opens the fridge, takes out a can of cola, and pulls the tab with a "pop." He takes a sip, walks back to the sofa, sits down, and picks up his phone.

Xiao Jie (calmly): 妈，您放心。

He raises his phone higher, panning the camera across the corner of the living room — where two friends, A Dai and A Gua, are sitting. A Dai is watching TV while doing a handstand. A Gua is wearing socks on his hands like gloves, clumsily peeling an orange with his "sock hands."

Xiao Jie (serious expression): 我不交不三不四的朋友。

The shot cuts to a close-up: the corners of Xiao Jie's mouth curl up slightly.

Xiao Jie (adding): 所以我的朋友——都很二。

Freeze frame: A Dai makes a peace sign (but upside down, it looks like the number "two"), while A Gua simultaneously holds up two fingers in a "V" sign. Sunlight hits their fingers perfectly.
...
Fade out."""
        ...
        # 生成视频（LTX2是纯文本到视频，不需要图片输入）
        result = video_generator.generate_video(
            workflow_path=workflow_path,
            image_path=None,  # LTX2不需要图片输入
...
def generate_video_from_prompt_and_url(prompt, image_url, negative_prompt=None, duration_seconds=5):
    """
    对外提供的简化接口，只需要传入提示词即可生成视频并返回结果（LTX2纯文本到视频）
    
    参数:
        prompt (str): 视频生成的提示词
        image_url (str): 输入图片的URL（LTX2不需要图片输入，此参数保留用于兼容性）
        negative_prompt (str, optional): 负面提示词，默认为通用设置
        duration_seconds (int, optional): 视频时长（秒），默认5秒
...
def generate_video_from_local_image(image_path, prompt, negative_prompt=None):
    """从本地图片生成视频（LTX2纯文本到视频，此函数保留用于兼容性）
    
    注意：LTX2是纯文本到视频，不需要图片输入，此函数仅用于兼容性
    """
    # 创建视频生成器实例
    video_generator = ComfyUIVideoGenerator()
